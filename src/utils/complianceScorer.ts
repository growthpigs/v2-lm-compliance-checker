import { AxeResults } from '@axe-core/puppeteer';
import { CanSpamResult } from './complianceRules';

interface ComplianceScoreInput {
  axeResults: AxeResults;
  hasPrivacyPolicy: boolean;
  barMention: string | null;
  canSpamResult: CanSpamResult;
}

interface ComplianceScoreDetails {
  score: number;
  breakdown: {
    accessibility: number;
    privacy: number;
    barAssociation: number;
    canSpam: number;
  };
  recommendations: string[];
}

export function calculateComplianceScore(input: ComplianceScoreInput): ComplianceScoreDetails {
  const weights = {
...
function calculateAccessibilityScore(axeResults: AxeResults): number {
  if (!axeResults.violations.length) return 100;
...
function calculateCanSpamScore(result: CanSpamResult): number {
  let score = 0;
  if (result.hasContactInfo) score += 50;
  if (result.hasUnsubscribeInfo) score += 50;
  return score;
}

function generateRecommendations(input: ComplianceScoreInput): string[] {
  const recommendations: string[] = [];

  // Add accessibility recommendations
  if (input.axeResults.violations.length > 0) {
    recommendations.push(
      'Fix accessibility violations to improve WCAG compliance',
      ...input.axeResults.violations.slice(0, 3).map(v => 
        `Fix ${v.impact} issue: ${v.help}`
      )
    );
  }
