...
// Interface for AI content detection results
interface AIContentDetectionResult {
  detectedAIContent: boolean;
...
export async function checkAIContentDisclosure(page: Page): Promise<AIContentDetectionResult> {
  try {
    ...
    const disclosureXPath = disclosurePatterns.map(pattern => 
      `contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '${pattern}')`
    ).join(' or ');
...
// Interface for plagiarism detection results
interface PlagiarismCheckResult {
  suspectedPlagiarism: boolean;
  riskLevel: 'high' | 'medium' | 'low';
  contentSamples: string[];
  details: string[];
}
...
export async function checkForPlagiarism(page: Page): Promise<PlagiarismCheckResult> {
  try {
    ...
    content.forEach(paragraph => {
      ...
      const isBoilerplate = legalBoilerplate.some(boilerplate => 
        paragraph.toLowerCase().includes(boilerplate.toLowerCase())
      );
